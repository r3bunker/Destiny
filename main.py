...
if modes == 'Subclass':
    ...
    def get_access_token():
        if "access_token" not in st.session_state:
            oauth_url = f"https://www.bungie.net/en/OAuth/Authorize?client_id={CLIENT_ID}&response_type=code"
            st.markdown("### Authentication Required")
            st.markdown(f"1. [Click here to authenticate with Bungie]({oauth_url})")
            st.markdown("2. Log in and authorize the app")
            st.markdown("3. Copy the full URL after authorization")
            st.markdown("4. Paste the URL below and click 'Submit'")
            
            full_url = st.text_input("Enter the full URL:")
            if st.button("Submit"):
                if full_url:
                    ...
                    if code:
                        ...
                        st.warning("The URL does not contain an authorization code.")
                else:
                    st.warning("Please enter the full URL.")
            return None
